...
func TestAppendMilestoneAsSet(t *testing.T) {
	var tests = []struct {
		...
		{
			[]github.Milestone{},
			&github.Milestone{ID: 42},
			[]github.Milestone{github.Milestone{ID: 42}}},
		{
			[]github.Milestone{github.Milestone{ID: 42}},
			&github.Milestone{ID: 42},
			[]github.Milestone{github.Milestone{ID: 42}}},
		{
			[]github.Milestone{github.Milestone{ID: 42}},
			&github.Milestone{ID: 43},
			[]github.Milestone{github.Milestone{ID: 42}, github.Milestone{ID: 43}}},
	}

	for _, test := range tests {
		got := appendMilestoneAsSet(test.set, test.milestone)
		if len(got) != len(test.want) {
			t.Errorf("len(appendMilestoneAsSet(%q, %q)) = %d, want %d",
				test.set, test.milestone, len(got), len(test.want))
		} else {
			for idx, gotValue := range got {
				if gotValue != test.want[idx] {
					t.Errorf("appendMilestoneAsSet(%q, %q)[%d] = %q, want %q",
						test.set, test.milestone, idx, gotValue, test.want[idx])
				}
			}
		}
...
func TestIncluesMilestone(t *testing.T) {
	var tests = []struct {
...
func TestAppendUserAsSet(t *testing.T) {
	var tests = []struct {
		...
		{
			[]github.User{github.User{ID: 42}},
			&github.User{ID: 42},
			[]github.User{github.User{ID: 42}}},
		{
			[]github.User{github.User{ID: 42}},
			&github.User{ID: 43},
			[]github.User{github.User{ID: 42}, github.User{ID: 43}}},
	}
	for _, test := range tests {
		got := appendUserAsSet(test.set, test.user)
		if len(got) != len(test.want) {
			t.Errorf("len(appendUserAsSet(%q, %q)) = %d, want %d",
				test.set, test.user, len(got), len(test.want))
		} else {
			for idx, v := range got {
				if v != test.want[idx] {
					t.Errorf("appendUserAsSet(%q, %q)[%d] = %q, want %q",
						test.set, test.user, idx, v, test.want[idx])
				}
			}
		}
