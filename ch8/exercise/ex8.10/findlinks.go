...
var (
	sema        = make(chan struct{}, 20) // counting semaphore for limiting concurrency.
...
func Extract(url string, cancel <-chan struct{}) ([]string, error) {
	...
	if err != nil {
		return []string{}, err // NOTE: why not nil,err ?
	}

	// NOTE: Set channel for cancel request.
	req.Cancel = cancel
